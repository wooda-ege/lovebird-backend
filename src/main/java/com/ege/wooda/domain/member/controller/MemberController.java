...
public class MemberController {
    ...
    public ResponseEntity<ApiResponse<Long>> add(@RequestPart(value = "images") List<MultipartFile> images,
                                                 @Validated @RequestPart(value = "memberCreateRequest") MemberCreateRequest memberCreateRequest) throws IOException {
        Long addMemberId = memberService.save(images, memberCreateRequest);
        return new ResponseEntity<>(
                ApiResponse.createSuccessWithData(MemberResponseMessage.CREATE_MEMBER.getMessage(), addMemberId),
                HttpStatus.CREATED);
    }

    @GetMapping("/{nickname}")
    public ResponseEntity<ApiResponse<MemberDetailResponse>> details(@PathVariable String nickname) {
        MemberDetailResponse memberDetailResponse = memberService.findMemberByNickname(nickname).toMemberDetailResponse();
        return ResponseEntity.ok(ApiResponse.createSuccessWithData(MemberResponseMessage.READ_MEMBER.getMessage(), memberDetailResponse));
    }

    @PutMapping("/{nickname}")
    public ResponseEntity<ApiResponse<Long>> modify(@PathVariable String nickname,
                                                    @RequestPart(value = "images") List<MultipartFile> images,
                                                    @Validated @RequestPart(value = "memberUpdateRequest") MemberUpdateRequest memberUpdateRequest) throws IOException {
        Long modifyMemberId = memberService.update(nickname, images, memberUpdateRequest);
        return ResponseEntity.ok(ApiResponse.createSuccessWithData(MemberResponseMessage.UPDATE_MEMBER.getMessage(), modifyMemberId));
    }
